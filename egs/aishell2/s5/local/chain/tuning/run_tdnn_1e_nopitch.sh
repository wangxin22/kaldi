...
# _1e_nopitch is as _1e, but with no pitch features

set -euxo pipefail

# configs for 'chain'
affix=all_plus_rir_nopitch
stage=10
train_stage=-10
get_egs_stage=-10
train_set=train
test_sets="dev test"
feat_dir=data/${train_set}_hires_nopitch
...
if [[ $rir == "true" ]] && [ -d data/${train_set}_hires_nopitch_clean_plus_rir ]; then
  feat_dir=data/${train_set}_hires_nopitch_clean_plus_rir
fi
...
if [ $stage -le 6 ]; then
  ...
  for datadir in ${train_set}; do
    steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 data/${datadir}_hires_nopitch_clean_plus_rir data/${datadir}_hires_nopitch_clean_plus_rir_max2
